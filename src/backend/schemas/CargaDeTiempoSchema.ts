...
export const CargaDeTiempoSchema = z.object({
    horas: z.number().min(0, { message: "Las horas no pueden ser negativas" }).max(23, { message: "Las horas no pueden ser mayor a 24" }),
...
}).refine((cargaDeTiempo) => !(cargaDeTiempo.horas === 0 && cargaDeTiempo.minutos === 0), "No se puede cargar horas y minutos con 0");
