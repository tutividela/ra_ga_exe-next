...
const nuevaPrenda = async (req: NextApiRequest, res: NextApiResponse) => {
  try {
    const {
      ...
      precioMuyComplejo,
      precioUltraComplejo,
      precioExtremadamenteComplejo,
    } = NewClothingCategorySchema.parse(req.body);

    const [
      precioBasicoObj,
      precioMedioObj,
      precioComplejoObj,
      precioMuyComplejoObj,
      precioUltraComplejoObj,
      precioExtremadamenteComplejoObj,
    ] = await prisma.complejidadConfeccion.findMany({
      where: {
        OR: [
          { name: "Básico" },
          { name: "Medio" },
