...
export default async function post(req: NextApiRequest, res: NextApiResponse) {
  try {
    const { idEstadoProcesoDesarrolloOrden, idUsuario } =
      RegistroEstadoProcesoDesarrolloOrdenSchema.parse(req.body);
    const { id } = req.query;
    const idProcesoDesarrolloOrdenString = Array.isArray(id) ? id[0] : id;

    const registroGuardado =
      await prisma.registroCambioEstadoProcesoDesarrolloOrden.create({
        data: {
