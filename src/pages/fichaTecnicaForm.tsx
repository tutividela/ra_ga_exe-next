...
const Home: NextPage = () => {
  ...
  const { data: clothesData } = useQuery<TipoPrenda[], ErrorMessage>(
    ["clothes"],
    getClothes,
    { refetchOnWindowFocus: false, onError: (error) => addError(error.error) }
  );

  const { mutateAsync: uploadFilesMutation, isLoading: isUploading } =
    useMutation<DriveUploadResponse, ErrorMessage, FileUploadData>(uploadFile, {
      onError: (error) => addError(error.error),
    });

  const { mutateAsync: createOrderMutation, isLoading: isCreatingOrder } =
    useMutation<{ message: string }, ErrorMessage, OrderCreationData>(
      createOrder,
      {
        onSuccess: (obj) => {
          router.replace("/");
  ...
  const advanceStep = () =>
    step < 4
      ? setStep((prev) => prev + 1)
      : alert("No se puede ir mass para adelante");
  const goBackOneStep = () =>
    step > 0
      ? setStep((prev) => prev - 1)
      : alert("No se puede ir mas para atras");
  ...
  const handleUploadFile = async (
    file: { file: File; section: Paths<OrderCreationData> }[],
    orderID: string
  ) => {
    ...
    <div className="bg-split-white-black">
      ...
      <main>
        <Slide in={true} timeout={500} direction="up">
          <div>
            <LoadingIndicator
              show={isFetching || isMutating || isCreatingOrder || isUploading}
            >
                ...
                >
                  <div className="flex flex-col">
                    <div className="md:mt-9 grow flex justify-evenly">
                      ...
                      />
                      {step === 0 && (
                        <ClothingSelectionForm clothesData={clothesData} />
                      )}
                    ...
                    <div className="flex justify-self-end justify-center md:justify-end w-full md:w-10/12 space-x-4 mt-7 md::mb-7 md:mt-24">
                      <FichaTecnicaControls
                        currStep={step}
                        numberSteps={steps.length}
                        onBack={goBackOneStep}
                        onForward={advanceStep}
                      />
                    </div>
                  </div>
                </HookForm>
