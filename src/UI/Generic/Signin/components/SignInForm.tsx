...
const SignInForm = (props: SignInFormProps) => {
  ...
  const loginSubmit = async (data: SignInSchemaType) => {
    try {
      ...
      if (availableData?.available) {
        ...
        if (res.error) {
          const errorMessage =
            (JSON.parse(res.error)?.error as string) || "Login Invalido";
          const message =
            typeof errorMessage === "object" ? "Login Invalido" : errorMessage;
  ...
  return (
    <HookForm
      ...
      formOptions={{ resolver: zodResolver(SignInSchema) }}
    >
