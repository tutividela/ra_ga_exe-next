  ...
  >(sendEmail, {
    onError: (error) => {
      ...
      if (Object.keys(error).length === 0)
        addError("Error al enviar el correo");
    },
    onSuccess: () => {
      addError("Correo enviado!", "success"), onClose();
    },
  });
    ...
    >
      <LoadingIndicator show={loading || loadingRecovery}>
        <div className="container mx-auto flex flex-col items-center rounded-none">
          <PageTitle
            title={emailRecovery ? "Recuperar contraseña" : "Iniciar sesión"}
            size="small"
          />
          <div className="mt-10" ref={containerRef}>
            ...
            >
              <div>
                {emailRecovery && (
                  <RecoveryForm
                    sent={!!emailData?.message}
                    onClose={onClose}
                    onRecovery={handleChangeToRecovery}
                    onSubmit={emailRecoverySubmit}
                  />
                )}
              </div>
            </Slide>
