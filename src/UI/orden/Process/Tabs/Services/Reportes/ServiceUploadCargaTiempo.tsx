...
import {
  crearCargaDeTiempoPorIdProcesoDesarrolloOrden,
  actualizarCargaDeTiempoPorId,
} from "@utils/queries/registros";
...
export default function ServiceUploadCargaTiempo({
  open,
  onClose,
  nombreProceso,
  ...
  const {
    mutateAsync: cargarTiempoAsync,
    isLoading: seEstaCreandoUnaCargaDeTiempo,
  } = useMutation<
    CargaDeTiempo & { usuarioDeCreacion: { name: string } },
    any,
    Partial<CargaDeTiempo>
  >(crearCargaDeTiempoPorIdProcesoDesarrolloOrden, {
  ...
  const {
    mutateAsync: editarTiempoAsync,
    isLoading: seEstaEditandoUnaCargaDeTiempo,
  } = useMutation(actualizarCargaDeTiempoPorId, {
    onError: (error) => addError(JSON.stringify(error), "error"),
    onSuccess: () => {
      onClose();
      queryClient.invalidateQueries(["cargasDeTiempo"]);
  ...
  async function handleCargaDeTiempo(
    data: Partial<CargaDeTiempo> & { email: string }
  ) {
    await cargarTiempoAsync(data);
  }

  async function handleEditarDeTiempo(
    data: Partial<CargaDeTiempo> & { email: string }
  ) {
  ...
  return (
    <Dialog open={open} onClose={onClose} fullWidth={true}>
      <DialogTitle className="self-center text-2xl m">
        Carga de tiempo para: {nombreProceso}
      </DialogTitle>
      <LoadingIndicator
        show={seEstaCreandoUnaCargaDeTiempo || seEstaEditandoUnaCargaDeTiempo}
      >
        <HookForm
          defaultValues={defaultLayout}
          onSubmit={
            TipoDeAccioModal.EDITAR === accion
              ? handleEditarDeTiempo
              : handleCargaDeTiempo
          }
        >
          <DialogContent>
