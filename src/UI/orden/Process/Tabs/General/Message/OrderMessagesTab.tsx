import {
  OrderMessageSchema,
  OrderMessageSchemaType,
} from "@backend/schemas/OrderMessageSchema";
...
const createMessage = async (
  data: OrderMessageSchemaType
): Promise<Mensaje> => {
  return fetch("/api/order/post-message", {
    method: "POST",
...
const OrderMessagesTab = ({ orderData, selectedProcess }: Props) => {
  const [loadingMessage, setLoadingMessage] = useState<ShownMessage>({
    id: "temp",
    message: "",
    timestamp: "",
    user: { email: "", name: "" },
  ...
  const { data: messages } = useQuery(
    ...
    () => (orderID ? getOrderMessages(orderID) : ([] as ShownMessage[])),
    { initialData: [], refetchOnWindowFocus: false }
  );

  const { mutateAsync, isLoading: creatingMessage } = useMutation(
    createMessage,
    {
      onSuccess: () => {
  ...
  const handleSendMessage = async (messageData: OrderMessageSchemaType) => {
    ...
    const mensaje = await mutateAsync({
      ...messageData,
  ...
  return (
    <div className="flex flex-col mt-4">
      <div className="m-4 border-4 shadow-lg min-h-[16rem] max-h-[20rem] overflow-y-scroll flex p-4 flex-col space-y-2">
        {messages
          .filter((mess) => mess.section === selectedProcess)
          .map((mess) => (
            <OrderMessageItem
              key={mess.id}
              message={mess}
      ...
      <div className="mx-4">
        <HookForm
          ...
          formOptions={{ resolver: zodResolver(OrderMessageSchema) }}
          resetOnSubmit
        >
