...
const OrderImageItem = ({ archivo }: Props) => {
  ...
  const handleDownload = async () => {
    ...
    await downloadFromFetch(
      `/api/drive/download?file=${archivo.urlID}`,
      archivo.name
    );
  ...
  return (
    <div className="my-3 mr-4 flex flex-col space-y-4">
        ...
        <div className="border-2 p-4 w-fit transition-all duration-400">
          <div className="flex flex-col items-center">
            <div className="self-start">
              <Button type="button" onClick={handleImageHide}>
      ...
      >
        ...
        {!showImage && (
          <MenuItem onClick={handleImageDisplay}>Visualizar</MenuItem>
        )}
        {showImage && (
          <MenuItem onClick={handleImageHide}>Cerrar visualización</MenuItem>
        )}
      </Menu>
    </div>
  );
