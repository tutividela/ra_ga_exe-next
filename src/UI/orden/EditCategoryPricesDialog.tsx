...
export default function EditCategoryPricesDialog(props: ConfirmDialogProps) {
  ...
  const {
    data: singleClothingPriceData,
    isFetching: isFetchingSingleClothingPriceData,
  } = useQuery<PrecioPrendaExtended, ErrorMessage>(
    ["singlePriceData", props.idToShow],
    () => (props.idToShow ? getSinglePrice(props.idToShow) : placeHolderData),
    {
    ...
    <div>
      ...
      >
        <div className="p-4">
          <DialogTitle>
            {"Modificación precio " +
              singleClothingPriceData.tipo.name +
              " " +
              singleClothingPriceData.complejidad.name || ""}
          </DialogTitle>
          <LoadingIndicator show={isFetchingSingleClothingPriceData}>
            {!isFetchingSingleClothingPriceData && (
              <HookForm
                defaultValues={singleClothingPriceData}
                onSubmit={handleNewClothingSubmit}
                formOptions={{
                  resolver: zodResolver(ModifyClothingPriceSchema),
                }}
              >
