...
const ServicesPricesTab = () => {
  ...
  const [confirmEditServicePriceOpen, setConfirmEditServicePriceOpen] =
    useState(false);
  ...
  const { data: servicesData, isFetching: isFetchingServicesData } = useQuery<
    Servicio[],
    ErrorMessage
  ...
  const EditButton = (id: string) => (
    <IconButton
      type="button"
      onClick={() => handleEditServicePriceDialog(id || "")}
    >
  ...
  const columns: GridColumns = [
    ...
    {
      field: "precioFijo",
      headerName: "Precio fijo",
      minWidth: 150,
      flex: 2,
      valueGetter: (params) => params?.row?.precioFijo.toFixed(2),
    },
    {
      ...
      minWidth: 150,
      flex: 2,
      renderCell: (params) => params?.row?.factorMultiplicador.toFixed(5),
    },
    {
      field: " ",
      headerName: "Acciones",
  ...
  return (
    <LoadingIndicator show={isFetchingServicesData || isMutating}>
      <EditServicePriceDialog
        onClose={handleCloseEditServicePriceDialog}
        open={confirmEditServicePriceOpen}
