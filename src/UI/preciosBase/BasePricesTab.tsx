...
const BasePricesTab = () => {
  ...
  const { data: allClothesPrices, isFetching: isFetchingData } = useQuery(
    ["clothesAndPrices"],
    getAllClothesPrices,
    {
  ...
  const EditButton = (id: string) => (
    <IconButton
      type="button"
      onClick={() => handleEditCategoryDialog(id || "")}
    >
  ...
  const columns: GridColumns<PrecioPrendaExtended> = [
    {
      ...
      minWidth: 150,
      flex: 2,
      valueGetter: (params) => params.row.tipo.name,
    },
    {
      field: "complejidad",
      ...
      minWidth: 150,
      flex: 2,
      valueGetter: (params) => params.row.complejidad.name,
    },
    {
      field: "precioBase",
      ...
      flex: 2,
      renderCell: (params) =>
        `${currencyFormatter.format(params?.row?.precioBase || 0)}`,
    },
    {
      field: " ",
      headerName: "Acciones",
