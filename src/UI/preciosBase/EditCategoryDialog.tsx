...
export default function EditCategoryDialog(props: ConfirmDialogProps) {
  ...
  const {
    data: clothingAndPriceData,
    isFetching: isFetchingClothingAndPriceData,
  } = useQuery<TipoPrendaExtended, ErrorMessage>(
    ["clothingAndPriceData", props.idToShow],
    () => getClothingAndPrices(props.idToShow),
    {
        ...
        <div className="p-4">
          ...
          <LoadingIndicator show={isFetchingClothingAndPriceData}>
            {!isFetchingClothingAndPriceData && (
              <HookForm
                defaultValues={clothingAndPriceData}
                onSubmit={handleNewClothingSubmit}
                formOptions={{
                  resolver: zodResolver(ModifyClothingCategorySchema),
                }}
              >
