...
const NewClothesTab = () => {
  ...
  const [confirmDeleteClothingOpen, setConfirmDeleteClothingOpen] =
    useState(false);
  const [focusedItem, setFocusedItem] = useState("");

  const { data: clothesData, isFetching: isFetchingClothesData } = useQuery<
    TipoPrenda[],
    ErrorMessage
  >(["clothes"], getClothes, {
  ...
  const TableActions = (item: TipoPrenda) => (
    <div className="flex flex-row space-x-4">
      <div>
        <IconButton
          type="button"
          onClick={() => handleEditCategoryDialog(item.id)}
        >
      ...
      <div>
        <IconButton
          type="button"
          onClick={() => handleDeleteCategoryDialog(item.id)}
        >
          <DeleteIcon />
        </IconButton>
      </div>
  ...
  const columns: GridColumns<TipoPrenda> = [
    { field: "name", headerName: "Nombre", minWidth: 150, flex: 1 },
    {
      field: " ",
      headerName: "Acciones",
      renderCell: (params) => TableActions(params.row),
      filterable: false,
      sortable: false,
      align: "center",
      headerAlign: "center",
  ...
  return (
    <LoadingIndicator show={isFetchingClothesData || isMutating}>
      <AddNewCategoryDialog
        onClose={handleCloseNewCategoryConfirmDialog}
        open={confirmNewClothingOpen}
      />
      <EditCategoryDialog
        onClose={handleCloseEditCategoryDialog}
        open={confirmEditPricesOpen}
        idToShow={focusedItem}
      />
      <DeleteCategoryDialog
        onClose={handleCloseDeleteCategoryDialog}
        open={confirmDeleteClothingOpen}
        idToDelete={focusedItem}
      />
