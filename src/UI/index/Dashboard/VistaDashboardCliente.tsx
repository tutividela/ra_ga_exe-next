...
import {
  ReducedUserInfoSchema,
  ReducedUserInfoSchemaType,
} from "@backend/schemas/ReducedUserInfoSchema";
...
const VistaDashboardCliente = (props: props) => {
  ...
  const { data: orderData, isLoading: isFetchingOrders } = useQuery(
    ["ordenes", props?.emailToFetchOrders],
  ...
  return (
    <>
                ...
                <div className="h-full border-2 flex justify-center items-center">
                  <div className="flex flex-col space-y-4 items-center">
                    <div className="text-2xl">
                      Actualmente no tiene ordenes de desarrollo activas
                    </div>
                    <div>
                      <Link href={"/fichaTecnicaForm"}>
                        <Button variant="outlined" startIcon={<PostAddIcon />}>
      ...
      <MobileOrderDashboard
        ...
        orderData={orderData}
        userId={reducedUserInfo?.user.id}
      />
    </>
  );
};
