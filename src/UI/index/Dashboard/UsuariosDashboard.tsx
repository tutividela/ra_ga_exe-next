...
const UsuariosDashboard = () => {
  ...
  const [confirmEditUserAvailabilityOpen, setConfirmEditUserAvailabilityOpen] =
    useState(false);
  const [openSignUp, setOpenSignUp] = useState(false);
  const [openSignIn, setOpenSignIn] = useState(false);
  const [focusedItem, setFocusedItem] = useState<ReducedUser>({
    id: "",
    name: "",
    email: "",
    role: { name: "" },
  ...
  const columns: GridColumns = [
    ...
    {
      field: "switchPermissions",
      headerName: "",
      maxWidth: 80,
      align: "center",
      headerAlign: "center",
      disableColumnMenu: true,
      renderCell: (params) => EditRoleButton(params.row.id, params.row.email),
    },
    {
      field: "available",
    ...
    },
    {
      ...
      maxWidth: 80,
      renderCell: (params) =>
        EditAvailableButton(params.row.id, params.row.email),
    },
  ];
  ...
  const EditAvailableButton = (id: string, email: string) => (
    <IconButton
      type="button"
      onClick={() =>
        handleEditUserAvailabilityDialog({
          id,
          email,
          name: "",
          role: { name: "" },
  ...
  return (
    <div>
      <EditUserRoleDialog
        onClose={handleCloseEditUserRoleDialog}
        open={confirmEditUserRoleOpen}
        idToShow={focusedItem?.id}
        email={focusedItem?.email}
      />
      <EditUserAvailabilityDialog
        onClose={handleCloseEditUserAvailabilityDialog}
        open={confirmEditUserAvailabilityOpen}
        idToShow={focusedItem?.id}
        email={focusedItem?.email}
      />
