...
export default function EditServicePriceDialog(props: ConfirmDialogProps) {
  ...
  const { data: servicePriceData, isFetching: isFetchingServicePriceData } =
    useQuery<PrecioServicioExtended, ErrorMessage>(
      ...
      () =>
        props.idToShow ? getServicePrice(props.idToShow) : placeHolderData,
      {
    ...
    <div>
      ...
      >
        {servicePriceData?.name === "Precio del Dolar" ? (
          ...
          <div className="p-4">
            <DialogTitle>
              {"Modificación " + servicePriceData?.name || ""}
            </DialogTitle>
            <LoadingIndicator show={isFetchingServicePriceData}>
              {!isFetchingServicePriceData && (
                <HookForm
                  ...
                  onSubmit={handleNewClothingSubmit}
                  formOptions={{
                    resolver: zodResolver(ModifyPreciosServiciosSchema),
                  }}
                >
                  <DialogContent className="space-y-5">
