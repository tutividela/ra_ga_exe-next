...
export default function EditCategoryPricesDialog(props: ConfirmDialogProps) {
  ...
  const { data: allRolesData, isFetching: isFetchingAllRoles } = useQuery<
    Role[],
    ErrorMessage
  ...
  const roles = useMemo(
    () => allRolesData?.map((el) => ({ key: el.name, text: el.name })) || [],
    [allRolesData]
  );

  const { data: roleData, isFetching: isFetchingRoleData } = useQuery<
    Role,
    ErrorMessage
  >(
    ["userRole", props.idToShow],
    () => (props?.email ? getRole(props?.email) : null),
    {
      refetchOnWindowFocus: false,
  ...
  const { mutateAsync: modifyRoleMutation } = useMutation<
    Role,
    ErrorMessage,
    Role
  ...
  return (
    <div>
      ...
      >
        <div className="p-4">
          <DialogTitle>
            {"Modificación del rol a: " + props?.email || ""}
          </DialogTitle>
