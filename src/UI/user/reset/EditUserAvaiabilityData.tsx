...
export default function EditUserAvailabilityDialog(props: ConfirmDialogProps) {
  ...
  const { data: availabilityData, isFetching: isFetchingAvailabilityData } =
    useQuery<{ available: boolean }, ErrorMessage>(
      ["userAvailability", props.idToShow],
      () => (props?.email ? getAvailability(props?.email) : null),
      {
  ...
  const { mutateAsync: modifyAvailabilityMutation } = useMutation<
    { available: boolean },
    ErrorMessage,
    { available: boolean }
  ...
  return (
    <div>
      ...
      >
        <div className="p-4">
          <DialogTitle>
            {"Activar o desactivar: " + props?.email || ""}
          </DialogTitle>
            ...
            {!isFetchingAvailabilityData && (
              ...
              >
                <DialogContent className="space-y-5">
                  <FormItem
                    layout={editUserAvailabilityLayout}
                    selectOptions={{
                      availabilities: [
