...
function Uploader<Model>(props: UploaderProps<Model>) {
  ...
  const sectionFiles = useMemo(
    () =>
      allFiles
        .filter((el) => el.section === fileInfoScope)
        .map((el) => ({
          name: el.file.name,
          type: el.file.type,
          urlID: "test.url",
        })),
    [fileInfoScope, allFiles]
  );
              ...
              <div className="flex flex-row items-center">
                ...
                >
                  <input
                    ...
                    hidden
                    accept="image/*,.plt,.jpg,.png,.pdf"
                    type="file"
                    onChange={(event) =>
                      onChange([
                        ...value?.filter(
                          (el) => el.section !== layout?.options?.fileSection
                        ),
                        ...Array.from(event.target.files).map((el) => ({
                          file: el,
                ...
                <div className="border-2 w-full mx-2 md:mx-4 p-4 flex flex-col md:flex-row md:flex-wrap overflow-x-hidden rounded-sm border-gray-300">
                  {value?.map(
                    (el: { file: File; section: string }, i: number) => {
                      if (el.section === layout?.options?.fileSection)
                        return (
                          <FileInfoTag
                            file={el.file}
                            key={i}
