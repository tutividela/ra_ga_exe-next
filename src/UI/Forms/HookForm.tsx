...
export default function HookForm<TFieldValues, TContext = any>(
  props: HookFormProps<TFieldValues, TContext>
) {
  const {
    onSubmit,
    defaultValues,
  ...
  useEffect(() => {
    if (resetOnSubmit && formState.isSubmitSuccessful) {
      reset();
    }
  }, [resetOnSubmit, formState.isSubmitSuccessful, reset]);

  useEffect(() => {
    if (resetOnDialogClose && resetOnDialogClose.dialogStatus === false) {
      reset();
    }
  ...
  return (
    <FormProvider {...formContext}>
      <form onSubmit={formContext.handleSubmit(onSubmit)}>{children}</form>
    </FormProvider>
  );
}
