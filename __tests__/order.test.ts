...
describe("Orders Testing", () => {
  ...
  it("Should create a new order with correct values", async () => {
    const prenda = await prisma.tipoPrenda.findFirst({
      where: { name: "Pantalón" },
    });
  ...
  it("Should create a new order with the selected services and generate the correct Price", async () => {
    ...
    const precioBase = await prisma.precioPrenda.findFirst({
      where: { tipoId: prenda.id },
    });
    const precioDolar = await prisma.precioDelDolar.findFirst({
    ...
    expect(status.mock.calls[0][0]).toBe(200);
    expect(json.mock.calls[0][0].data.nombre).toBe(
      "Pantalón Básico con Precio"
    );
    expect(json.mock.calls[0][0].data.cotizacionOrden[0].precio).toBe(
      precioBase.precioBase * precioDolar.precio
    );
  });

  it("Should create multiple orders and be able to obtain them", async () => {
    const prenda_1 = await prisma.tipoPrenda.findFirst({
      where: { name: "Pantalón" },
    });
    const precioBase_1 = await prisma.precioPrenda.findFirst({
      where: { tipoId: prenda_1.id },
    });

    const prenda_2 = await prisma.tipoPrenda.findFirst({
      where: { name: "Remera / Camiseta" },
    });
    const precioBase_2 = await prisma.precioPrenda.findFirst({
      where: { tipoId: prenda_2.id },
    });
    ...
    expect(json_1.mock.calls[0][0].data.nombre).toBe(
      "Pantalón Básico con Precio"
    );
    expect(json_1.mock.calls[0][0].data.cotizacionOrden[0].precio).toBe(
      precioBase_1.precioBase * precioDolar.precio
    );
    ...
    expect(json_2.mock.calls[0][0].data.nombre).toBe(
      "Remera Básica con Precio"
    );
    expect(json_2.mock.calls[0][0].data.cotizacionOrden[0].precio).toBe(
      precioBase_2.precioBase * precioDolar.precio
    );
