...
describe("User Testing", () => {
  ...
  it("Should not create a User with incorrect email values", async () => {
    ...
    expect(json.mock.calls[0][0].error?.fieldErrors?.email?.[0]).toBe(
  ...
  it("Should not create a User with incorrect password values", async () => {
    ...
    expect(
      json.mock.calls[0][0].error?.fieldErrors?.confirmPassword?.length
    ).toBe(1);
    expect(json.mock.calls[0][0].error?.formErrors?.length).toBe(1);
    expect(json.mock.calls[0][0].error?.formErrors?.[0]).toBe(
      "Las contraseñas deben ser iguales"
    );
    expect(json.mock.calls[0][0].error?.fieldErrors?.password?.[0]).toBe(
      "Se requiere un mínimo de 8 caracteres"
    );
    expect(json.mock.calls[0][0].error?.fieldErrors?.confirmPassword?.[0]).toBe(
      "Se requiere un mínimo de 8 caracteres"
    );
  });
  ...
  it("Should not create a User with incorrect name values", async () => {
    ...
    expect(json.mock.calls[0][0].error?.fieldErrors?.name?.[0]).toBe(
      "Se requiere un mínimo de 1 caracter"
    );
  });
  ...
  it("Should validate correct inputs for credential check", async () => {
    ...
    expect(json.mock.calls[0][0].error?.fieldErrors?.username?.[0]).toBe(
      "Formato de correo electrónico inválido"
    );
    expect(json.mock.calls[0][0].error?.fieldErrors?.password?.length).toBe(1);
    expect(json.mock.calls[0][0].error?.fieldErrors?.password?.[0]).toBe(
