...
describe("Clothes Testing", () => {
  ...
  it("Should create a new clothing type", async () => {
    ...
    } = generateMockRes<CreateClothesResponse>();
    const req_create = { method: "POST", body: data } as NextApiRequest;

    const {
      res: res_obtainClothes,
      json: json_obtainClothes,
      status: status_obtainClothes,
    } = generateMockRes<ObtainClothesResponse>();
    const {
      res: res_obtainPrices,
      json: json_obtainPrices,
      status: status_obtainPrices,
    } = generateMockRes<ObtainPricesResponse>();
  ...
  it("Should check complexities for a specific clothing type", async () => {
    ...
    const {
      res: res_obtainComplexities,
      json: json_obtainComplexities,
      status: status_obtainComplexities,
    } = generateMockRes<ComplejidadConfeccion[]>();
    const req_obtainComplexities = {
      query: { idPrenda: json_create.mock.calls[0][0].prenda.id },
    } as unknown as NextApiRequest;

    await getComplexitiesByClothesID(
      req_obtainComplexities,
      res_obtainComplexities
    );
